...
const HELP := {
	...
	"peak_alpha_tarmac": "TYRE, LATERAL (cornering) - slip ANGLE where lateral grip peaks on asphalt. Low = tarmac bite that arrives early and lets go sharply.",
	"peak_alpha_gravel": "TYRE, LATERAL (cornering) - slip ANGLE where lateral grip peaks on gravel. Higher than tarmac: gravel slides deeper and more progressively before letting go. This gap IS the surface difference.",
	...
	"cy_gravel": "TYRE - the same falloff shape for GRAVEL. Lower it to make slides catchable without moving where grip peaks (that is Grip peak LAT gravel). This is the knob for 'the car lets go too easily once sideways'.",
	"trail_pneumatic": "STEERING FEEL - pneumatic trail at zero slip: how far behind the contact centre the cornering force acts. Sets how heavy the wheel gets before it goes light. This is the trail that COLLAPSES at the grip peak.",
	"trail_mechanical": "STEERING FEEL - fixed trail from caster geometry. Unlike pneumatic trail it does NOT collapse, so it sets how much weight is left in the wheel once the front is sliding.",
	"steer_ratio": "STEERING FEEL - steering ratio (wheel turns : road-wheel turns). Higher = slower rack and a lighter reported torque.",
	"steer_filter_tau": "STEERING FEEL - how fast the steering system itself can respond (seconds). A real column, rack and pair of arms cannot follow an 8 ms load spike, so this filters impulses out while leaving steady cornering torque untouched. 0 = raw per-tick moment, for A/B.",
	"sat_gain": "STEERING FEEL - output scaling on the reported steering torque. 0 = signal off; this is display/rumble scaling only and does not change how the car drives.",
	...
	"stage_seed": "STAGE - the SHAKEDOWN stage's seed. Every corner, crest and cut derives from it: change it for a completely different stage, change it back to get the first one again exactly. The stage rebuilds about half a second after you stop moving the slider.",
	"stage_sinuosity": "STAGE - how much of its turn budget the SHAKEDOWN road spends. 0 runs almost straight across the area, 1 is as twisty as the design speed allows. It cannot ask for a corner tighter than the minimum radius.",
	"stage_elevation": "STAGE - SHAKEDOWN's elevation character. 0 keeps the road on the valley floor, 1 sends it climbing over ridges. Raising it also makes the road lean harder around high ground rather than climbing it.",
	"stage_chunk_budget_ms": "STAGE - how much of each physics tick (8.33 ms at 120 Hz) SHAKEDOWN's ground may spend building itself. The stage is streamed in chunks now, and a chunk's cost is almost entirely working out where the road is at each vertex - so a chunk is built a slice at a time rather than all at once, and THIS is the size of the slice. Everything else derives from it: raise it and the ground is solid further ahead of you but the worst frame grows; lower it and frames are calmer while the ground is built closer. If you ever feel ground appear late at high speed, raise this.",
	"stage_design_speed": "STAGE - the speed SHAKEDOWN is DESIGNED for, in km/h, which fixes its tightest corner via R = V^2/(127(e+f)). RAISING THIS MAKES THE STAGE STRAIGHTER AND SHORTER, not faster - a rally stage is a road driven far beyond its design speed, and that mismatch is the point.",
	"tyre_audio_surface": "AUDIO - where the tyre sound gets its SURFACE. ON asks the ground map (gravel sounds like gravel). OFF is the old grip threshold, which read the rally loop as 40% asphalt - a tarmac squeal under every gravel slide and the gravel rumble cut 28%. Use Tyre gain if the corrected rumble is now too loud.",
	"damper_reads_road": "SUSPENSION - whether the damper feels the ROAD's own vertical speed, not just the body's. ON is the physical answer and is what makes surface texture reach you; OFF reverts to roughness pushing through the spring alone (the original C1 behaviour, which could not be felt).",
	...
	"peak_slip_tarmac": "TYRE, LONGITUDINAL (drive/brake) - the slip ratio where TARMAC grip peaks (~0.14). Lower = grip arrives and leaves sooner, so the limit feels more on/off.",
	"peak_slip_gravel": "TYRE, LONGITUDINAL (drive/brake) - the slip ratio where GRAVEL grip peaks (~0.28). Much later than tarmac, which is why loose surfaces reward wheelspin.",
...
var _specs := [
	...
	["Grip peak LONG tarmac (M2)","peak_slip_tarmac",       0.06,     0.45,  0.01, func(v): return "%d%% slip" % int(v * 100.0)],
	["Grip peak LONG gravel (M2)","peak_slip_gravel",       0.10,     0.60,  0.01, func(v): return "%d%% slip" % int(v * 100.0)],
	...
	["Grip peak LAT tarmac (M2)","peak_alpha_tarmac", 4.0,     16.0,  0.5,  func(v): return "%.1f deg" % v],
	["Grip peak LAT gravel (M2)","peak_alpha_gravel", 6.0,     24.0,  0.5,  func(v): return "%.1f deg" % v],
	...
	["Trail pneumatic (M2)", "trail_pneumatic",        0.0,      0.08, 0.005, func(v): return "%.1f cm" % (v * 100.0)],
	["Trail mechanical (M2)","trail_mechanical",       0.0,      0.08, 0.005, func(v): return "%.1f cm" % (v * 100.0)],
	["Steering ratio (M2)",  "steer_ratio",            8.0,     24.0,  0.5,  func(v): return "%.1f:1" % v],
	["Steer response (M2)",  "steer_filter_tau",       0.0,      0.20, 0.005, func(v): return ("RAW" if v < 0.0001 else "%.3f s" % v)],
	["Steer torque gain (M2)","sat_gain",              0.0,      2.0,  0.1,  func(v): return "%.1fx" % v],
	...
	["Stage streaming budget", "stage_chunk_budget_ms", 0.25, 4.0,  0.25, func(v): return "%.2f ms/tick" % v],
